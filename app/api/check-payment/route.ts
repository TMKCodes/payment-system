...
function buildConfirmedPayments(address: string, utxos: Array<{
    address: string;
    outpoint: { transactionId: string; index: number };
...
function buildPendingPayments(address: string, entries: Array<{
    address: string;
    receiving: Array<{
        transaction: {
    ...
    return entries
        ...
        .flatMap((entry) =>
            entry.receiving.map((receivingEntry) => {
                ...
                const totalSompi = matchingOutputs.reduce(
...
async function sweepMerchantFunds(
    ...
    const totalInput = signingUtxos.reduce(
    ...
    builder
        .addOutput(destinationAddress, outputAmount.toString())
        .setFee(fee);
...
async function findNetworkConfirmedPayment(
    ...
    for (const payment of payments.sort(
        (left, right) => Number(BigInt(right.blockDaaScore) - BigInt(left.blockDaaScore)),
    )) {
        ...
        const confirmations = Number(
            currentVirtualDaaScore - BigInt(blockResult.result.header.daaScore),
        );
...
async function findConfirmedTrackedCandidatePayment(
    ...
    for (const payment of payments) {
        const txStatusResult = await client.getTransactionStatus(
    ...
    try {
        ...
        if (!utxosResult.ok || !utxosResult.result) {
            return NextResponse.json(
                { error: utxosResult.error ?? 'Failed to query address UTXOs' },
                { status: 502 },
            );
        }
        ...
        if (!mempoolResult.ok || !mempoolResult.result) {
            return NextResponse.json(
                { error: mempoolResult.error ?? 'Failed to query mempool entries' },
                { status: 502 },
            );
        }
        ...
        const session =
            existingSession &&
                ...
                existingSession.amountSompi === expectedAmountSompi
                ? existingSession
                : {
                    address,
        ...
        const maybeSubmitSweepTransaction = async () => {
            ...
            session.sweepTransactionHash = await sweepMerchantFunds(
                client,
                merchantPrivateKey,
                sweepAddress,
            );
        ...
        const latestConfirmedPayment = await findNetworkConfirmedPayment(
            client,
            address,
            [
                ...
                ...trackedConfirmedPayments.filter(
                    (payment) =>
                        !matchingConfirmedPayments.some(
                            (candidate) => candidate.transactionHash === payment.transactionHash,
                        ),
                ),
            ],
        );
        ...
        const latestConfirmedTrackedCandidate = latestConfirmedPayment
            ? null
            : await findConfirmedTrackedCandidatePayment(
                client,
                address,
                expectedAmountSompi,
                trackedCandidatePayments,
            );
        ...
        const latestUnconfirmedObservedPayment = matchingConfirmedPayments.sort(
            (left, right) => Number(BigInt(right.blockDaaScore) - BigInt(left.blockDaaScore)),
        )[0] ?? null;
